...
class Game {
    constructor(canvas, extent, assets) {
        ...
        this.snakearray = [
//            new SnakeSegment (2, 4, this.cellSize, this.context, this.assets), 
            this.snakehead
        ];
...
//Check for collision with apple
    applecollision(lastx, lasty){
        if (this.snakehead.x == this.apple.x && this.snakehead.y == this.apple.y) {
            ...
            this.apple.x = Math.floor((Math.random() * this.extent));
            this.apple.y = Math.floor((Math.random() * this.extent));
        }
    }
...
//Check for collision with apple
    ...
    if (this.snakehead.x < 0 || 
        this.snakehead.y < 0 || 
        this.snakehead.x >= this.extent ||
        this.snakehead.y >= this.extent) {
            this.gameOver();
        }
    }
 ...
 //Checking for collision with snake body
    snakecollision(){
        for (let i=0; i < (this.snakearray.length-1); i++){
            if (this.snakearray[i].x == this.snakehead.x &&
                this.snakearray[i].y == this.snakehead.y) {
